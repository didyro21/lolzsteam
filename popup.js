...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const cbShowInv = document.getElementById('settingShowInv');
    const cbDebug = document.getElementById('settingDebug');
    const inputApiKey = document.getElementById('settingApiKey');

    // Retrieve exactly from storage and set defaults
    chrome.storage.local.get(['autoScan', 'autoSkip', 'showInv', 'debugMode', 'apiKey'], (res) => {
        if(cbAutoScan) cbAutoScan.checked = res.autoScan !== false; 
        if(cbAutoSkip) cbAutoSkip.checked = res.autoSkip === true; 
        if(cbShowInv) cbShowInv.checked = res.showInv !== false; // Defaults to TRUE
        if(cbDebug) cbDebug.checked = res.debugMode === true;    
        if(inputApiKey) inputApiKey.value = res.apiKey || '';
    });

    // Save exactly to storage on change
    if(cbAutoScan) cbAutoScan.addEventListener('change', () => chrome.storage.local.set({ autoScan: cbAutoScan.checked }));
    if(cbAutoSkip) cbAutoSkip.addEventListener('change', () => chrome.storage.local.set({ autoSkip: cbAutoSkip.checked }));
    if(cbShowInv) cbShowInv.addEventListener('change', () => chrome.storage.local.set({ showInv: cbShowInv.checked }));
    if(cbDebug) cbDebug.addEventListener('change', () => chrome.storage.local.set({ debugMode: cbDebug.checked }));
    if(inputApiKey) inputApiKey.addEventListener('input', (e) => chrome.storage.local.set({ apiKey: e.target.value.trim() }));
    ...
    const findTradeBansBtn = document.getElementById('findTradeBansBtn');
    if(findTradeBansBtn) {
        findTradeBansBtn.addEventListener('click', async () => {
            ...
            findTradeBansBtn.textContent = "🔍 Scanning...";
            findTradeBansBtn.disabled = true;
            ...
            try {
                ...
                findTradeBansBtn.textContent = "🔍 Find Perm Trade Bans on Page";
                findTradeBansBtn.disabled = false;
            }
        });
    }
    ...
    try {
        ...
        if (transactions.length > 0 && toggleBtn) {
        ...
        if (tab.url.includes("lzt.market") && !tab.url.includes("lzt.market/steam") && !tab.url.includes("lzt.market/item")) {
             ...
             document.getElementById('amount').textContent = "Welcome, this is a beta build. If any errors or issues, let me know. Thanks!";
        }
    }
});

// --- INJECTED FUNCTION: Scans page for Trade Bans ---
function scanForTradeBans() {
    ...
    items.forEach(item => {
        ...
        if (banNode) {
            ...
            if (titleLink) {
                foundAccounts.push({ title: titleLink.innerText.trim(), url: titleLink.href });
            }
        }
    });
...
function updateMainUI(totalUSD, hasUncertain) {
    ...
    if(amountDiv) amountDiv.textContent = `External spent: $${totalUSD.toFixed(2)}`;
    if(actWarningDiv) actWarningDiv.style.display = isRecentActivity ? 'inline-block' : 'none';
    ...
    if(warningDiv) warningDiv.textContent = hasUncertain ? "Needs manual data parsing for currency" : "";
}
